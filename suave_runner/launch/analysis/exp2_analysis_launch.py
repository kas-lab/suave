...
"""Launch paired Wilcoxon analysis for a sorted exp2 campaign."""
...
def generate_launch_description():
    """Configure the paired analysis node from a sorted exp2 campaign."""
    share = Path(get_package_share_directory('suave_runner'))
    config = share / 'config' / 'analysis' / 'exp2_analysis_config.yml'
    return LaunchDescription([
        DeclareLaunchArgument(
            'results_root',
            description='Directory with run_idx-sorted CSVs for exp2, '
                        'e.g. the sort_results.py --output directory or '
                        'a batch campaigns/exp2/sorted/ directory'),
        DeclareLaunchArgument(
