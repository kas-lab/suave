...
"""Launch paired Wilcoxon analysis for a sorted exp1 campaign."""
...
def generate_launch_description():
    """Configure the paired analysis node from a sorted exp1 campaign."""
    share = Path(get_package_share_directory('suave_runner'))
    config = share / 'config' / 'analysis' / 'exp1_analysis_config.yml'
    return LaunchDescription([
        DeclareLaunchArgument(
            'results_root',
            description='Directory with run_idx-sorted CSVs for exp1, '
                        'e.g. the sort_results.py --output directory or '
                        'a batch campaigns/exp1/sorted/ directory'),
        DeclareLaunchArgument(
