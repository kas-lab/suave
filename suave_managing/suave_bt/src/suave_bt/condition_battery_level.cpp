...
namespace suave_bt
{
BatteryLevel::BatteryLevel(const std::string & xml_tag_name, const BT::NodeConfig & conf)
: BT::ConditionNode(xml_tag_name, conf), battery_level_(-1.0)
{
  ...
  diagnostics_sub_ = node_->create_subscription<diagnostic_msgs::msg::DiagnosticArray>(
    "/diagnostics", 10, std::bind(&BatteryLevel::diagnostics_cb, this, _1));
}
...
BT::NodeStatus BatteryLevel::tick()
{
  ...
  if (battery_level_ >= threshold || battery_level_ == -1.0) {
